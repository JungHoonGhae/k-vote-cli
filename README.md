...
<div align="center">
  <h1>k-vote-cli</h1>
  <p><strong>선거 결과, 누구나 직접 받아볼 수 있게 — 한국 선거 공개 데이터를 한 명령으로.</strong></p>
  <p>개표결과·여론조사를, 특별한 권한이나 가입 없이. 사람이든 AI든 같은 명령(<code>kvote</code>)으로 받아 바로 봅니다.</p>
  <p><sub>역대 핵심 개표결과 <strong>한 번에 다운로드 + 투표구별 정리</strong> · 여론조사 전수 수집 · 투표율·당선인 · 인증키 자동 발급 — <a href="#기능">전체 기능표 ↓</a></sub></p>
</div>
...
선거 결과를 내 눈으로 직접 확인하고 싶어도, 막상 그 데이터를 받아보는 건 쉽지 않습니다.
분명 "공개"돼 있다는데 — **어디서 받는지 막막하고, 받아도 파일이 안 열리고, 결국 개발자가
아니면 포기**하게 됩니다.

`k-vote-cli` 는 그 과정을 **한 명령**으로 줄입니다. 흩어진 선거 공개 데이터(개표결과·여론조사)를,
특별한 권한이나 가입 없이, 누구나 — 그리고 AI도 — 바로 쓸 수 있는 형태로 받습니다.
