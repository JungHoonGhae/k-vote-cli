...
> [!WARNING]
> **일부 기능**(여론조사 수집, 데이터셋 검색·다운로드 경로)은 공개 포털의 HTML 구조에 의존해,
> 사이트 개편 시 해당 기능이 깨질 수 있습니다 — `kvote doctor` 로 바로 점검할 수 있습니다.
> 파일 파싱·로컬 DB·MCP 는 사이트 개편과 무관하게 동작합니다. 수집 대상은 **법적 공개 의무가
> 있는 공공 데이터**(선거여론조사기준에 따른 등록·공개 자료)이며, 기본 rate limit 을 적용해
> 예의 있게 수집합니다. 본인 책임 하에 사용하세요.
